    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-xs text-slate-400 mb-1">
                ...
                <span>&bull;</span>
                <span class="text-slate-600 font-medium">{{ $user->name }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Modifier le Compte Utilisateur</h1>
        </div>
    ...
    <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Nouveau mot de passe (laisser vide si inchangé)</label>
                    <input type="password" name="password" placeholder="••••••••" class="w-full py-2.5 px-3 bg-slate-50 text-xs rounded-xl border border-slate-200 focus:bg-white focus:border-sky-500 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Rôle dans le système *</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Lier à un collaborateur</label>
